...
| M4 | fixed-unverified | Tasdiqlandi: `PaginationFilter::wrap()` da `DB::select($totalQuery)` **umuman binding olmasdi**; sanalar 6 joyda xom interpolatsiya qilinardi. **Halollik uchun:** bu hozirda `date_format:Y-m-d` validatsiyasi bilan niqoblangan, ya'ni **latent**, tirik eksploit emas. Baribir binding'ga o'tkazildi. **Regressiya testlari bilan qoplandi** (`tests/Feature/ReportFilterTest.php`, `tests/Feature/PaginationFilterTest.php`): sanalar va `author_id` SQL matnida emas, binding sifatida ketishi; total so'rov ham AYNI bindinglarni olishi; `perPage` chegaralanishi; raqam bo'lmagan `page` xato bermasligi. So'rovlar bajarilmaydi - `DB::select` ushlab qolinadi, chunki bu SQL PostgreSQL'ga xos. **Baseline kodda bu testlarning 36 tasi xato beradi, tuzatilgandan keyin hammasi o'tadi.** SQL'ning O'ZI PostgreSQL'da hali bajarilmadi. |
...
| **Hisobot turi hech qachon tanlanmasdi.** `ReportFilterByBenefit` va `ReportFilterByBooks` da `$formRequest->post('report_type_id') === ReportTypeEnum::X->value` yozilgan edi: chap tomon **satr**, o'ng tomon **int**, ya'ni `"7" === 7` har doim `false`. Natijada saralash doim `asc` bo'lgan, `BOOKS_FREE` va `BOOKS_NOT_BOUGHT` shohobchalari esa **umuman bajarilmagan**. `ReportService::purchaseStatistics()` allaqachon `intval()` ishlatgani niyatni tasdiqlaydi. | Yuqori | Tuzatildi va testlar bilan qoplandi (`ReportFilterTest`) |
